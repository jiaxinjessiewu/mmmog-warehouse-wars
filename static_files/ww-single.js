...
Stage.prototype.initialize=function(){
    ...
    for(var y=0; y < this.height; y++){
        ...
        for(var x=0; x < this.width; x++){
            if((x==cw && y == ch) || false){
                s+="<td><img id="+this.getStageId(x,y)+" src="+this.meImageSrc+" style='height:25'' width:25'/></td>";
                player = new Player(this, y, x);
                this.addActor(player);
                this.player=player;
//            }else if (x==1 && y==1){
//                s+="<td><img id="+this.getStageId(x,y)+" src="+this.devilImageSrc+" style='height:25'' width:25'/></td>";
//                devil = new Devil(this, x, y);
//                this.addActor(devil);
            }else{
                var prob = Math.random();
                if(x == 0 || y == 0 || x == this.width-1 || y == this.height-1 ){
                    s+="<td><img id="+this.getStageId(x,y)+" src="+this.wallImageSrc+" style='height:25'' width:25'/></td>";
                    wall = new Wall(this, x, y);
                    this.addActor(wall);
                    
                } else if(prob < 0.07 && prob > 0.02){
                    s+="<td><img id="+this.getStageId(x,y)+" src="+this.monsterImageSrc+" style='height:25'' width:25'/></td>";
                    monster = new Monster(this, x, y);
                    this.addActor(monster);
                } else if(prob < 0.4 && prob > 0.07){
                    s+="<td><img id="+this.getStageId(x,y)+" src="+this.boxImageSrc+" style='height:25'' width:25'/></td>";
                    box = new Box(this, x, y);
                    this.addActor(box);
                }else {
                    s+="<td><img id="+this.getStageId(x,y)+" src="+this.blankImageSrc+" style='height:25'' width:25'/></td>";
                }
            }
        }
        s+="</tr>";
...
//    console.log("100: "+"("+actor.x+","+actor.y+")");
    document.getElementById("("+actor.x+","+actor.y+")").src = this.blankImageSrc;
    this.actors.splice(index,1);
}

// Set the src of the image at stage location (x,y) to src
Stage.prototype.setImage=function(x, y, src){
//    console.log("107: "+this.getStageId(x,y));
...
//Player.prototype.updateMove=function(){this.ismove = true;}
...
Monster.prototype.step=function(){
    if(this.checkDeath()==true){
        ...
        while (blockElement == false){
            ...
            var dest = this.stage.getActor(this.x + this.xSteps[randMove], this.y + this.ySteps[randMove]);
            if (!(dest instanceof Box || dest instanceof Wall || dest instanceof Monster)){
                this.move(this, this.xSteps[randMove], this.ySteps[randMove]);
                blockElement = true;
...
Box.prototype.move=function(asker,x,y){
    ...
    if((asker instanceof Player || asker instanceof Box) && (dest == null || dest.move(this,x,y))){
        this.stage.removeActor(this);
        this.x = newx;
        this.y = newy;
